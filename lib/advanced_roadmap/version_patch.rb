  ...
  module VersionPatch
    def self.included(base)
      base.class_eval do
        ...
        def issueCache(cache = nil)
          if cache.nil?
            return @issueCacheObject
          else
            if @issueCacheObject.nil?
              @issueCacheObject = Array.new
            end
            @issueCacheObject << cache
          end
        end
        ...
        def closed_spent_hours
          if !@closed_spent_hours
            ...
            if !@issueCacheObject.nil?
              @issueCacheObject.each do |issue|
                if issue.closed?
        ...
        def calculate_advance_info
          ...
          if fixed_issues.size > 0
            fixed_issues.each do |issue|
              ...
              # issue.children is checking the database, and seems to also just ignore the parent's hours.
            ...
            if solved_issues < Setting.plugin_redmine_advanced_roadmap["solved_issues_to_estimate"].to_i or total_solved_estimated == 0.0
              @progress_factor = nil
            else
        ...
        def sorted_issues(parent, issues = {})
          sorted_issues = []
          if !@issueCacheObject.nil?
            @issueCacheObject.each do |issue|
              if issue.parent_issue_id == parent
                sorted_issues << issue
                sorted_issues += sorted_issues(issue,issues)
              end
            end
          end
          sorted_issues
